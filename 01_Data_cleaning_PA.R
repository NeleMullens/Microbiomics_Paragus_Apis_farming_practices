...
#When needed, install packages first:
#phyloseq and dada2
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft = c(17, 21),
                     truncLen = c(260, 210), maxN = 0, maxEE = c(1, 1),
                     truncQ = 2, rm.phix = TRUE, compress = TRUE)
# On non-Windows set multithread = TRUEhead(out)
...
seqtab.nochim2 <- removeBimeraDenovo(seqtab, method = "consensus",
...
taxa <- assignTaxonomy(seqtab.nochim2,
                       "data/SILVA_138.1_SSURef_NR99_tax_silva.0.fa",
                       multithread = FALSE)

input_taxa <- read.csv("outputs/Input_taxonomic_assignment.csv",
...
taxa_assigned_extra <- assignTaxonomy(input_taxa_matrix,
                                      "data/SILVA_data_DNA.fa",
                                      multithread = TRUE)
...
write.table(
  table,
  file = "outputs/Table_with_reads_and_taxonomic_data_updated.txt",
  sep = "\t",
  col.names = TRUE,
  row.names = TRUE
)
...
write.table(table,
            file = "outputs/Table_with_bacteria.txt",
            sep = "\t",
            col.names = TRUE,
            row.names = TRUE)
...
# add extra columns containing taxa, for this, first transform the taxa object
# to a data frame
...
write.table(table, "outputs/Full_table_with_taxonmy_other_ref.txt",
...
# transform Full_table_with_taxonmy.txt: add a first column with OTUs, and the
# negative control(s) should be next. Remove taxonomic data (or reduce to one
# column, at the end)
clean_table <- read.csv("outputs/Table_to_clean_negs.csv")
clean_tab <- as.data.frame(clean_table)
View(clean_tab)
# if error, check if there are empty columns and remove these
# through clean_tab$X <- NULL
contamdone <- decon(data = clean_tab, numb.blanks = 5,
                    numb.ind = c(4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
                                 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
                                 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4),
                    taxa = FALSE)
#all 96 samples in groups of 4; based on location, sex, and treatment

contamdone2 <- contamdone$decon.table
contamdone2  #with this step we can see which ASV have been removed due to too
             #low counts in samples compared to the negative control and how
             #many counts have been subtracted for the ASVs that have not been
             #removed

cleaned <- as.data.frame(contamdone2)
write.table(cleaned, file = "outputs/Filtered_OTUs.txt",
...
write.table(switched, file = "outputs/Filtered_OTUs.txt",
            sep = "\t", col.names = TRUE, row.names = TRUE)
...
# The last seven columns are for taxonomic identification, we want to
# select all but that
...
write.table(freq_table,
            "outputs/frequency_table_complete.txt",
            sep = "\t",
            col.names = TRUE,
            row.names = FALSE,
            quote = FALSE)
...
write.xlsx(freq_table,
           file = "outputs/frequency_table_taxonomy.xlsx",
           sheetName = "Sheet1",
           col.names = TRUE,
           row.names = TRUE,
           append = FALSE)
...
uniquesToFasta(seqtab.nochim2, "Sequences_Nov2022.fasta",
               ids = (1:ncol(seqtab.nochim2)))
...
Biostrings::writeXStringSet(Aligned_seq,
                            "outputs/Microbiome_Syrph_Apis_aligned.fasta")
...
rename <- read.csv("outputs/Rename_fasta_file.csv", header = TRUE)
ref_name <- data.frame(rename$ï..old_name, rename$new_name)
rename.fasta(infile = "outputs/Microbiome_Syrph_Apis_aligned.fasta",
             ref_table = ref_name,
             outfile = "outputs/Microbiome_Syrph_Apis.fasta")

## now we can continue with correct names
# use fasttree on the HPC, very fast
# Alternatively, use this tree with CIPRES for RAxML analysis,
# Geneious for consensus
...
obj <- newMRexperiment(abund_dat, phenoData = phenotypeData,
                       featureData = OTUdata)
...
write.csv(mat_df,
          file = "outputs/MetagenomeSeq/MetagenomeSeq_normalised_all_data.csv")
...
raw_microbiome_clr <- aldex.clr(raw_microbiome,
                                mc.samples = 500, denom = rclr, verbose = TRUE)
...
write.table(vegan_freq, file = "outputs/Vegan_frequency/vegan_normfreq.txt",
            sep = "\t", quote = FALSE, col.names = NA)


## ----Redo everything with aggregated genera. First, generate genera file -----

GENUS_abundances <- read.csv("outputs/Complete_filtered_data_genera.csv",
                             row.names = 1)
...
# 22002 OTUs representing 843 genera from 248 families and 30 phyla 
# (including NAs pooled for each family, clade or phylum)

# rowsum (pooling of genera: remove text columns from the matrix!)
ncol(GENUS_abundances)
colnames(GENUS_abundances)
temp_GENUS_abundances <- rowsum(GENUS_abundances[, 1:182],
                                GENUS_abundances$Genus, na.rm = TRUE)
...
ncol(GENUS_abundances)
colnames(GENUS_abundances)
temp_FAM_abundances <- rowsum(GENUS_abundances[, 1:182],
                              GENUS_abundances$Family, na.rm = TRUE)
...
ncol(GENUS_abundances)
colnames(GENUS_abundances)
temp_PHY_abundances <- rowsum(GENUS_abundances[, 1:182],
                              GENUS_abundances$Phylum, na.rm = TRUE)
...
# The last seven columns are for taxonomic identification, we want to select all
# but that
...
library(xlsx)
write.xlsx(freq_table, file = "outputs/freq_table_genera.xlsx",
...
raw_microbiome_clr <- aldex.clr(raw_microbiome, mc.samples = 500,
                                denom = rclr, verbose = TRUE)

ald_h <- lapply(X = raw_microbiome_clr@analysisData, FUN = rowMeans)
ald_h <- as.data.frame(ald_h)

write.xlsx(ald_h,
           file = "outputs/CentralLogTransformed/clr_normalised_genera.xlsx",
           sheetName = "Sheet1", col.names = TRUE,
...
mat_df <- as.data.frame(mat)
write.xlsx(mat_df, file = "outputs/MetagenomeSeq_normalised_genera.xlsx",
           sheetName = "Sheet1", col.names = TRUE,
           row.names = TRUE, append = FALSE)
...
write.xlsx(vegan_freq,
           file = "outputs/Vegan_frequency/vegan_normfreq_genera.xlsx",
           sheetName = "Sheet1", col.names = TRUE, row.names = TRUE, append = FALSE)
